...
func validatePodMarkerMatchRequirements(spec *appsv1alpha1.PodMarkerSpec, fldPath *field.Path) (allErrs field.ErrorList) {
	...
	podSelector, err := v1.LabelSelectorAsSelector(requirement.PodSelector)
	if err != nil {
	...
	} else if !util.IsSelectorOverlapping(requirement.PodSelector, &v1.LabelSelector{MatchLabels: spec.MarkItems.Labels}) {
		// in cases of podSelector.MatchLabels = {"a":"b"}, markItems.Labels = {"a":"c"}
		allErrs = append(allErrs, field.Invalid(fldPath.Child("podSelector"), *requirement.PodSelector,
			"podSelector fights with spec.markItems, spec.markItems will make podSelector invalid after marking"))
	}
	// 2. validate nodeSelector
	nodeSelector, err := v1.LabelSelectorAsSelector(requirement.NodeSelector)
	if err != nil {
	...
	podRequirements, _ := podSelector.Requirements()
	nodeRequirements, _ := nodeSelector.Requirements()
	if len(podRequirements)+len(nodeRequirements) == 0 {
		return append(allErrs, field.Invalid(fldPath, *requirement, "matchRequirements cannot be empty"))
	}
	return
}
