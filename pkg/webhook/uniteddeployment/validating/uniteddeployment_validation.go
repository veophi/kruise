...
func validateSubsetTemplate(template *appsv1alpha1.SubsetTemplate, selector labels.Selector, fldPath *field.Path) field.ErrorList {
	...
	if template.StatefulSetTemplate != nil {
		...
		allErrs = append(allErrs, appsvalidation.ValidatePodTemplateSpecForStatefulSet(coreTemplate, selector, fldPath.Child("advancedStatefulSetTemplate", "spec", "template"), apivalidation.PodValidationOptions{
			AllowDownwardAPIHugePages: true, AllowIndivisibleHugePagesValues: true, AllowInvalidPodDeletionCost: true,
		})...)
		...
		allErrs = append(allErrs, appsvalidation.ValidatePodTemplateSpecForReplicaSet(coreTemplate, selector, 0, fldPath.Child("deploymentTemplate", "spec", "template"), apivalidation.PodValidationOptions{AllowIndivisibleHugePagesValues: true, AllowDownwardAPIHugePages: true})...)
	}
