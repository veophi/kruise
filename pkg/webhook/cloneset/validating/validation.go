...
func (h *CloneSetCreateUpdateHandler) validateCloneSetSpec(spec, oldSpec *appsv1alpha1.CloneSetSpec, metadata *metav1.ObjectMeta, fldPath *field.Path) field.ErrorList {
	...
	if err != nil {
		...
		allErrs = append(allErrs, apivalidation.ValidatePodTemplateSpec(coreTemplate, fldPath.Child("template"), apivalidation.PodValidationOptions{AllowDownwardAPIHugePages: true, AllowIndivisibleHugePagesValues: true})...)
	}
