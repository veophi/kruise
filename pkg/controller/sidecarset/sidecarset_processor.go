...
func (p *Processor) UpdateSidecarSet(sidecarSet *appsv1alpha1.SidecarSet) (reconcile.Result, error) {
	...
	// turn the latest revision of sidecarSet.
	latestRevision, collisionCount, err := p.registerLatestRevision(sidecarSet, pods)
	if err != nil {
		klog.Errorf("sidecarSet get active revisions error, err: %v, name: %s", err, sidecarSet.Name)
		return reconcile.Result{}, err
	}
...
func (p *Processor) registerLatestRevision(sidecarSet *appsv1alpha1.SidecarSet, pods []*corev1.Pod) (
	...
	if equalCount > 0 && history.EqualRevision(revisions[revisionCount-1], equalRevisions[equalCount-1]) {
		...
		// when maxStoredRevisions is modified smaller,
		// this updated revision may be delete if we don't replace this revision.
		replaceRevision(revisions, equalRevisions[equalCount-1], latestRevision)
	} else {
		// if there is no equivalent revision we create a new one
		// in case of the latest change
...
func (p *Processor) truncateHistory(revisions []*apps.ControllerRevision, s *appsv1alpha1.SidecarSet, pods []*corev1.Pod) error {
	...
	limitation := integer.IntMax(1, maxStoredRevisions)
	...
	klog.V(3).Infof("Find revisions %v more than limitation %v, name: %v", revisionCount, limitation, s.Name)

	// the number of revisions need to delete
	deletionCount := revisionCount - limitation
	// only delete the revisions that no pods use.
	activeRevisions := getActiveRevisions(s, pods)
...
func getActiveRevisions(s *appsv1alpha1.SidecarSet, pods []*corev1.Pod) sets.String {
...
func calculateStatus(control sidecarcontrol.SidecarControl,
	pods []*corev1.Pod, latestRevision *apps.ControllerRevision, collisionCount int32,
	...
	return &appsv1alpha1.SidecarSetStatus{
		...
		RevisionHistoryLimit: int32(maxStoredRevisions),
	}
}
...
func inconsistentStatus(sidecarSet *appsv1alpha1.SidecarSet, status *appsv1alpha1.SidecarSetStatus) bool {
	return status.ObservedGeneration > sidecarSet.Status.ObservedGeneration ||
		...
		status.RevisionHistoryLimit != sidecarSet.Status.RevisionHistoryLimit ||
