...
func TestGetActiveRevisions(t *testing.T) {
	...
	if len(revisionList.Items) != maxStoredRevisions {
		t.Fatalf("in case of maxStoredRevisions: get wrong number of revisions, expected %d, atual %d", maxStoredRevisions, len(revisionList.Items))
	}
}
...
func TestTruncateHistory(t *testing.T) {
	...
	if len(list.Items) != 10 {
		t.Fatalf("expected revision len: %d, atual: %d", 10, len(list.Items))
	}
	for i := range list.Items {
		if i == 8 && list.Items[i].Name == getName(14) {
			continue
		} else if i == 9 && list.Items[i].Name == getName(15) {
			continue
		} else if i < 8 && list.Items[i].Name == getName(i+1) {
			continue
		}
		expected := i + 1
		switch i {
		...
		t.Fatalf("expected name %s, atual : %s", getName(expected), list.Items[i].Name)
	}
}
