...
func GetProportion(rs *apps.ReplicaSet, d apps.Deployment, deploymentReplicasToAdd, deploymentReplicasAdded, expectedReplicas int32) int32 {
	if rs == nil || *(rs.Spec.Replicas) == 0 || deploymentReplicasToAdd == 0 || deploymentReplicasToAdd == deploymentReplicasAdded {
		return int32(0)
	}

	rsFraction := getReplicaSetFraction(*rs, d, expectedReplicas)
...
func getReplicaSetFraction(rs apps.ReplicaSet, d apps.Deployment, expectedReplicas int32) int32 {
	// If we are scaling down to zero then the fraction of this replica set is its whole size (negative)
	if *(d.Spec.Replicas) == int32(0) {
		return -*(rs.Spec.Replicas)
	}

	expectedReplicas = expectedReplicas + MaxSurge(d)
	...
	newRSsize := (float64(*(rs.Spec.Replicas) * expectedReplicas)) / float64(annotatedReplicas)
...
func FindNewReplicaSet(deployment *apps.Deployment, rsList []*apps.ReplicaSet) *apps.ReplicaSet {
	...
	for i := range rsList {
		if EqualIgnoreHash(&rsList[i].Spec.Template, &deployment.Spec.Template) {
			...
			klog.V(3).Infof("Rollout -- Kruise found the newRS(%s/%s)", &rsList[i].Namespace, &rsList[i].Name)
			return rsList[i]
		}
	}
	klog.V(3).Infof("Rollout -- Kruise doesnt find the newRS")
