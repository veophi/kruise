...
type PodMarkerSpec struct {
	...
	// MatchRequirements describe the requirements that the marked Pods must satisfy.
	MatchRequirements PodMarkerRequirements `json:"matchRequirements"`
	// MatchPreferences defines the priority of Pods that could be marked;
	// MatchPreferences is an sequential list of pod priorities, where the Pods that satisfy
	// MatchPreferences[i] will have higher priority than MatchPreferences[j] if i < j.
	// If len(MatchPreferences) > 1, the sorting rule of Pods is similar to lexicographic order.
	MatchPreferences []PodMarkerPodPriority `json:"matchPreferences,omitempty"`
...
type PodMarkerStrategy struct {
	...
	// ConflictPolicy describe the behavior of PodMarker when some matched Pods that already have
...
type PodMarkerRequirements struct {
	// PodLabelSelector describe the pod label requirement
	PodSelector metav1.LabelSelector `json:"podSelector,omitempty"`
	// NodeLabelSelector describe the node label requirement
	NodeSelector metav1.LabelSelector `json:"nodeSelector,omitempty"`
}

// PodMarkerPodPriority describes what the Pods having this priority should look like.
type PodMarkerPodPriority struct {
